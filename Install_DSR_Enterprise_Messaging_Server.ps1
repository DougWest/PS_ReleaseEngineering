...
if ($DisplayName)
{
    ...
    {
        Write-Host "$DisplayVersion installed, but $env:Target_Version can only upgrade from 6.8.706.7045. Cancelling action."; exit 1
    }
    elseif (($env:Target_Version -EQ "6.8.905.7393") -AND ($DisplayVersion -NE "6.8.904.7382"))
    {
        Write-Host "$DisplayVersion installed, but $env:Target_Version can only upgrade from 6.8.904.7382. Cancelling action."; exit 1
    }
    elseif (($env:Target_Version -EQ "6.8.1007.7619"))
    {
        Write-Host "$DisplayVersion installed, but $env:Target_Version can only be installed bare. Cancelling action."; exit 1
    }
    elseif (($env:Target_Version -EQ "6.8.1217.7903") -AND ($DisplayVersion -NE "6.8.1008.7632"))
    {
        Write-Host "$DisplayVersion installed, but $env:Target_Version can only upgrade from 6.8.1008.7632. Cancelling action."; exit 1
    }
}
else
{
    if ($env:Target_Version -EQ "6.8.707.7052")
    {
        ...
        Write-Host "No DSR Enterprise Server installed, but $env:Target_Version can only upgrade from 6.8.706.7045. Cancelling action."; exit 1
    }
    elseif ($env:Target_Version -EQ "6.8.905.7393")
    {
        #Eventually, functionality should be added to install the base version for this patch and then apply the patch.
        Write-Host "No DSR Enterprise Server installed, but $env:Target_Version can only upgrade from 6.8.904.7382. Cancelling action."; exit 1
    }
}
...
if ("False" -EQ (test-path -path "${TargetWSPath}\$ZipFileName")){copy "${env:WORKSPACE}\$ZipFileName" "${TargetWSPath}\$ZipFileName"}
...
{
    Write-Host "NCR Enterprise Messaging Service not installed on $env:Target_Machine!"; Exit 1
}
